...
void	Animation::walk(Body &body, float time)
{
	...
	body.upperLeftArm.animation = applyJointRoation(body.upperLeftArm, rotate_z(armSwing));
	body.upperRightArm.animation = applyJointRoation(body.upperRightArm, rotate_z(-armSwing));
	body.upperLeftLeg.animation = applyJointRoation(body.upperLeftLeg, rotate_z(-legSwing));
	body.upperRightLeg.animation = applyJointRoation(body.upperRightLeg, rotate_z(legSwing));

	body.lowerLeftArm.animation = applyJointRoation(body.lowerLeftArm, rotate_z(armSwing * 0.5f));
	body.lowerRightArm.animation = applyJointRoation(body.lowerRightArm, rotate_z(-armSwing * 0.5f));
	body.lowerLeftLeg.animation = applyJointRoation(body.lowerLeftLeg, rotate_z(-legSwing * 0.5f));
	body.lowerRightLeg.animation = applyJointRoation(body.lowerRightLeg, rotate_z(legSwing * 0.5f));

	body.head.animation = applyJointRoation(body.head, rotate_z(sin(t * 0.5f) * 0.2f));
}
